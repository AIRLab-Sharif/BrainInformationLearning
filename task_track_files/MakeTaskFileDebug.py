...
for row in df_participants.iterrows():
    if row[1].Group == 'PD':
        ...
        tasks['P300_over_all_channels_without_normalization'] = np.append(tasks['P300_over_all_channels_without_normalization'], 'P300_over_all_channels__' +
                                                                          row[1].participant_id + '_ses-{0:0>2}_task-Rest_eeg_db.mat'.format(1))
        # tasks['PAC_dist_mean_channel'] = np.append(tasks['PAC_dist_mean_channel'],'PAC_mean_' + row[1].participant_id + '_ses-{0:0>2}_task-Rest_eeg.mat'.format(1))
...
if platform.system() == 'Linux':
    task_df.to_csv(
        './task_track_files/task_track_file_matlab_linux_double_calculation_Habituation.csv')
else:
    task_df.to_csv(
        './task_track_files/task_track_file_matlab_windows_double_calculation_Habituation.csv')
