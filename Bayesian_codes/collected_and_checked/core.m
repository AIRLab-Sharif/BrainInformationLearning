...
% Core is going to calculate \int P(s1:(t-1)|\theta) * log
% \frac{log(s_1:(t-1)|\theta)}{P(s_1:(t)|\theta)}
fun1=@(x,y) (y.^N(1,3)).*(x.^N(1,2)).*((1-x-y).^N(1,1));
fun2=@(x,y) (y.^N(2,3)).*(x.^N(2,1)).*((1-x-y).^N(2,2));
fun3=@(x,y) (y.^N(3,1)).*(x.^N(3,2)).*((1-x-y).^N(3,3));
% N_new = N * e^(-1/w) + 1{tr(1) is the power of parameters in P(s_1:(t)|\theta)
% N2 = N - N_new is the power of parameter in \frac{log(s_1:(t-1)|\theta)}{P(s_1:(t)|\theta)}
N2=(1-exp(-1/w))*N;
